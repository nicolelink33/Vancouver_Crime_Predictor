...
![Distribution of crime types](../results/figures/crime_type_distribution.png){#fig-type_dist width=80%}
...
Then, hyperparameter optimization was performed on all models, using a 15,000-observation stratified subset of the training data, in order to decrease computational load.
The best fit models found from these optimizations were then scored on the test data set, using both accuracy and F-1 score. 
F-1 score was selected due to the class imbalance seen in the dataset, as it is generally a more informative scoring metric on datasets with high class imbalances. 
...
After hyperparameter optimization, the optimal value was found to be k = 85, 
yielding a final accuracy of `{python} knn_scores['accuracy'].values[0]` 
and a final F1 score of `{python} knn_scores['f1'].values[0]` on the test set. 
The hyperparameter optimization results are shown in @fig-KNN_opt . 
![KNN hyperparameter optimization results](../results/figures/knn_k_optimization.png){#fig-KNN_opt width=80%}
...
![Confusion matrix for KNN classifier, showing predicted crime type versus actual crime type.](../results/figures/knn_confusion_matrix.png){#fig-KNN_cm width=100%}


### SVM
The baseline LinearSVC model with C = 1 achieved an accuracy of `{python} baseline_svm_scores['accuracy'].values[0]`. 
We then performed hyperparameter optimization on C, and fit the selected best model on the training data. 
This selected model gave a final accuracy of `{python} svm_scores['accuracy'].values[0]` 
...
![Confusion matrix for SVM classifier, showing predicted crime type versus actual crime type.](../results/figures/SVM_confusion_matrix.png){#fig-SVM_cm width=100%}


### Logistic Regression
The best fit logistic regression model achieved a test accuracy of `{python} baseline_logreg_scores['accuracy'].values[0]`, 
and an F1 score of `{python} baseline_logreg_scores['f1'].values[0]` indicating slightly lower performance than KNN and SVM. 
The confusion matrix for the best fit logistic regression model is shown in @fig-LR_cm . 
![Confusion matrix for Logistic Regression classifier, showing predicted crime type versus actual crime type.](../results/figures/log_reg_confusion_matrix.png){#fig-LR_cm width=100%}
...
- Baseline K-NN: `{python} baseline_knn_scores['accuracy'].values[0]`
- Optimized K-NN: `{python} knn_scores['accuracy'].values[0]`
- Optimized SVM: `{python} svm_scores['accuracy'].values[0]`
- Logistic Regression: `{python} baseline_logreg_scores['accuracy'].values[0]`

All three models also produced somewhat similar F1 scores, with the k-NN models performing best:

- Baseline K-NN: `{python} baseline_knn_scores['f1'].values[0]`
- Optimized K-NN: `{python} knn_scores['f1'].values[0]`
- Optimized SVM: `{python} svm_scores['f1'].values[0]`
- Logistic Regression: `{python} baseline_logreg_scores['f1'].values[0]`
...
Even with an average accuracy score of `{python} round((knn_scores['accuracy'].values[0] + svm_scores['accuracy'].values[0] + baseline_logreg_scores['accuracy'].values[0])/3, 2)`, these models could still be useful for helping police decide where to patrol,
