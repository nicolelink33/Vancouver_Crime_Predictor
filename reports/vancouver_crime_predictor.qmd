...
After tuning, all three models performed similarly, achieving an accuracy around `{python} round((knn_scores['accuracy'].values[0] + svm_scores['accuracy'].values[0] + baseline_logreg_scores['accuracy'].values[0])/3, 2)`. 
...
### Logistic Regression
The baseline logistic regression model ahieved a test accuracy of `{python} baseline_logreg_scores['accuracy'].values[0]`. The best fit logistic regression model achieved a test accuracy of `{python} logreg_scores['accuracy'].values[0]`
and an F1 score of `{python} baseline_logreg_scores['f1'].values[0]`,
indicating slightly lower performance than KNN and SVM., indicating slightly lower performance than KNN and SVM.
...
- Logistic Regression: `{python} logreg_scores['accuracy'].values[0]`
...
- Optimized Logistic Regression: `{python} baseline_logreg_scores['f1'].values[0]`
...
Even with an average accuracy of `{python} round((knn_scores['accuracy'].values[0] + svm_scores['accuracy'].values[0] + baseline_logreg_scores['accuracy'].values[0])/3, 2)` accuracy, these models could still be useful for helping police decide where to patrol,
